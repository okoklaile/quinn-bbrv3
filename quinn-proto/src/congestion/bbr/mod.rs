...
pub struct Bbr {
    ...
    loss_in_round: bool,
    ecn_in_round: bool,
    loss_events_in_round: u8,
    startup_ecn_rounds: u8,
}
...
impl Bbr {
    ...
    fn update_bw_probe(&mut self, _now: Instant, bytes_in_flight: u64) {
        match self.mode {
            Mode::ProbeBw => {
                if self.current_cycle_offset == BBR_BW_PROBE_UP {
                    if bytes_in_flight >= self.inflight_hi {
    ...
    fn update_ecn(&mut self, is_ce: bool) {
        if !self.ecn_eligible {
            return;
        }
        ...
        if is_ce {
            ...
            let gain = BBR_ECN_ALPHA_GAIN;
            self.ecn_alpha = ((BBR_UNIT - gain) * self.ecn_alpha + gain * BBR_UNIT) >> BBR_SCALE;
            
            let cut = BBR_UNIT - ((self.ecn_alpha * BBR_ECN_FACTOR) >> BBR_SCALE);
            self.inflight_lo = ((self.inflight_lo as u64 * cut as u64) >> BBR_SCALE) as u64;
        }
    }

    fn update_recovery(&mut self, bytes_lost: u64) {
        if bytes_lost > 0 {
            self.loss_in_round = true;
            
            let cut = BBR_UNIT - BBR_BETA;
            self.inflight_lo = ((self.inflight_lo as u64 * cut as u64) >> BBR_SCALE) as u64;
            self.bw_lo = ((self.bw_lo as u64 * cut as u64) >> BBR_SCALE) as u64;
        }
    }

    fn check_ecn_eligible(&mut self, rtt: Duration) {
        if !self.ecn_eligible {
            const MAX_ECN_RTT: Duration = Duration::from_millis(5);
            self.ecn_eligible = rtt <= MAX_ECN_RTT;
        }
    }
}
...
impl Controller for Bbr {
    ...
    fn on_ack(
        ...
        let bw = self.max_bandwidth.get_estimate();
        if !app_limited {
            self.max_bandwidth.update_bw_hi(bw);
        }
        ...
        if let Some(is_ce) = rtt.is_ce() {
            self.update_ecn(is_ce);
        }
        
        let in_flight = self.prev_in_flight_count;
        self.update_bw_probe(now, in_flight);
    }
    ...
    fn on_end_acks(
        ...
        // 在每个回合结束时更新带宽过滤器
        if is_round_start && !app_limited {
            self.max_bandwidth.advance_bw_hi_filter();
        }
    }
    ...
    fn on_congestion_event(
        ...
        self.update_recovery(lost_bytes);
    }
...
const BBR_ECN_ALPHA_INIT: u32 = BBR_UNIT;
const BBR_ECN_ALPHA_GAIN: u32 = BBR_UNIT / 16;
const BBR_ECN_FACTOR: u32 = BBR_UNIT / 3;
const BBR_BETA: u32 = BBR_UNIT * 30 / 100;

const BBR_BW_PROBE_UP: u8 = 7;
